...
public class KafkaProxyFrontendHandler
    ...
    public void inClientActive() {
        ...
        addFiltersToPipeline(netFilter.getFilterAndInvokerCollection(),
                clientCtx().pipeline(), clientCtx().channel(), routingContext);
    }
    ...
    /**
     * Called when connecting to a single cluster (backward compatible path).
     *
     * @param remote target broker
     * @param remoteSslContext optional TLS context
    ...
    public void inConnecting(
        ...
        // Connect the primary (only) cluster
        Channel inboundChannel = clientCtx().channel();
        connectionManager.connectPrimary(inboundChannel, remoteSslContext)
                .whenComplete((backend, t) -> {
                    if (t != null) {
                        LOGGER.warn("{}: Failed to connect to {}: {}",
                                sessionStateMachine.sessionId(), remote, t.getMessage());
                        sessionStateMachine.onClientException(t,
                                endpointBinding.endpointGateway().getDownstreamSslContext().isPresent());
                    }
                    else {
                        LOGGER.debug("{}: Connected to backend {}",
                                sessionStateMachine.sessionId(), backend.clusterId());
                ...
                .whenComplete((v, t) -> {
                    if (t != null) {
                        LOGGER.warn("{}: Failed to connect to all clusters: {}",
                                sessionStateMachine.sessionId(), t.getMessage());
                        // Don't fail immediately - some clusters might be connected
                        ...
                        LOGGER.info("{}: All {} clusters connected",
                                sessionStateMachine.sessionId(), clusters.size());
                    }
                    // Update decode predicate with filter requirements
                    dp.setDelegate(DecodePredicate.forFilters(filters));
                });
    ...
    /**
     * Called when the primary backend connection is established.
     * Forwards buffered messages and enables client auto-read.
     */
    public void onPrimaryBackendConnected() {
        LOGGER.debug("{}: Primary backend connected, forwarding {} buffered messages",
    ...
    public void initiateConnect(
        ...
        sessionStateMachine.onNetFilterInitiateConnect(remote, remoteSslContext, filters,
                virtualClusterModel, netFilter);
    }

    /**
     * Initiates connection to multiple servers.
     * Extended method for multi-cluster support.
     *
     * @param remotes map of clusterId to target address
     * @param sslContexts map of clusterId to TLS context
     * @param filters protocol filters
     */
    public void initiateMultiClusterConnect(
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("{}: Connecting to {} backend clusters using filters {}",
                    sessionStateMachine.sessionId(), remotes.size(), filters);
        }
        ...
        for (var entry : remotes.entrySet()) {
            ...
            clusters.put(entry.getKey(), new TargetCluster(
                    hp.host() + ":" + hp.port(),
